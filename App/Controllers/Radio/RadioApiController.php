...
class RadioApiController extends Controller {
	public function getInfo(ServerRequestInterface $request, ResponseInterface $response,Client $client){
		$radionomyResponse = $client->get("http://api.radionomy.com/currentsong.cfm?radiouid={$this->container->get('radionomy')['radio_uid']}&type=xml&cover=yes&defaultcover=yes&size=90&dynamicconf=yes ");
		...
		$endTime = Carbon::parse($track['start_time'])->addSeconds($track['play_duration'] / 1000);
